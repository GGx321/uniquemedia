import { z } from "zod";
import { AbsolutePath, ApiKey, EngineError, Id, Settings } from "../shared/engine";
...
export const HostControl = z.discriminatedUnion("type", [
  ...
  /**
   * Something main has to tell the windows (the engine restarted, settings.json
   * was corrupt). The engine emits it as an ordinary `engine.error` in its own
   * seq/bootId stream, so a window never sees an event from a foreign bootId.
   */
  z.strictObject({ kind: z.literal("control"), type: z.literal("notice"), error: EngineError }),
]);
...
export const HostCall = z.discriminatedUnion("type", [
   ...
   * main's dialog. Only the engine touches the library; main persists the new
   * path after an ok reply.
   */
