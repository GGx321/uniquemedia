...
const FILE_URL = "file:///Applications/Studio.app/Contents/Resources/app.asar/out-studio/renderer/index.html";
...
describe("isTrustedSender", () => {
  test("accepts the packaged renderer file, with or without a fragment or query", () => {
    expect(isTrustedSender(APP_FRAME, PACKAGED)).toBe(true);
    expect(isTrustedSender({ ...APP_FRAME, url: `${FILE_URL}#/avatars` }, PACKAGED)).toBe(true);
    expect(isTrustedSender({ ...APP_FRAME, url: `${FILE_URL}?x=1` }, PACKAGED)).toBe(true);
  });
  ...
  const rejected: [string, SenderFrame, TrustedRenderer][] = [
    ...
    ["another file", { ...APP_FRAME, url: "file:///tmp/evil/index.html" }, PACKAGED],
  ...
  test("compares file URLs as paths: percent-encoding and dot segments do not matter", () => {
    expect(isTrustedSender({ ...APP_FRAME, url: FILE_URL.replace("index.html", "index%2Ehtml") }, PACKAGED)).toBe(true);
    expect(isTrustedSender({ ...APP_FRAME, url: FILE_URL.replace("renderer/", "main/../renderer/") }, PACKAGED)).toBe(true);
    expect(isTrustedSender({ ...APP_FRAME, url: FILE_URL.replace("index.html", "index.html%2F..%2Fx") }, PACKAGED)).toBe(false);
  });
